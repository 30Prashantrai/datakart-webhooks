  ...
  handler.syncGCP = async (p) => {
    ...
    try {
      ...
      for (const field of requiredFields) {
        const value = field.split(".").reduce((o, key) => o?.[key], p);
        if (!value) {
          missingFields.push(field); // Add the missing field to the array
        }
      }

      if (missingFields.length > 0) {
        logData.status = "failed";
        logData.error = `${missingFields.join(", ")} are missing`;
        resp.status = false;
        resp.errorCode = "EZ-03";
        resp.message = `${missingFields.join(", ")} are missing`;
        resp.DK_2_0_Value_Success = `${resp.errorCode} ${resp.message}`;
        insertLog(logData);
        return resp;
      }
      ...
      if (p.gcp) {
        let company_tier = "basic";
        if (p.registration_type === "DataKart Registration") {
          company_tier = "premium";
        }
        let company_status = true;
        if (p.Company_status && p.Company_status !== "Active") {
        ...
        if (userresp.error) {
          ...
          return {
            status: false,
            message: "Error while Creating user details",
            errorCode: "EDK-02",
            DK_2_0_Value_Success: "EDK-02 Error while Creating user details",
          };
        }
        ...
        if (!companyresp.is_existing || companyresp.is_multiple_gcp) {
          ...
          //Gtin  , Gln & Gcn Allocation
          let company_gcp = {
            company_id: companyresp.data.company_id,
            gcp: p.gcp,
            info_type: info_type,
            company_tier: companyresp.data.company_id,
          };
          console.log("GCP, GTIN & GCN Allocation", company_gcp);
          let gtin = await gsonekeyServices.setGtins(company_gcp);
          if (gtin.error) {
            console.log(gtin);
            logData.status = "failed";
            logData.error = "Error in GCP, GTIN & GCN allocation";
            console.log("Error while GCP, GTIN & GCN allocation");
            insertLog(logData);
            gtin.status = false;
          ...
          if (company_tier === "basic") {
            template_for = "manufacturer_basic";
          } else {
            template_for = "manufacturer_premium";
          }
          if (companyresp.data.company_type == "retailer") {
            template_for = "retailer";
          }
