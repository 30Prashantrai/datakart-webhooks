      ...
      if (p.gcp) {
        ...
        if (companyresp.error) {
          ...
          if(companyresp.isEmailAccountIssue)
          {
            ...
            insertLog(logData);
            companyresp.status = false;
            companyresp.message = "Error due to same email for different account"
          ...
          else
          {
            ...
            insertLog(logData);
            companyresp.status = false;
            companyresp.message = "Company Creation Failed"
            console.log("Error while insert company details");
          }
        ...
        else if(!companyresp.is_existing)
        {
          ...
          if (userresp.error) {
            ...
            return { status: false, message: "Error while Creating user details" };
          }
          ...
          if (contactresp.error) {
            ...
            logData.status = "failed";
            logData.error = "Error Creating Company contact details - " + contactresp.message;
            console.log("Error Creating Company contact details");
            insertLog(logData);
            contactresp.status = false;
            contactresp.message = "Error Creating Company contact details"
            return contactresp;
          }
        }
        else
